...
async function generarExcelMatriz() {
    const formData = new FormData(document.getElementById('form-matriz'));
    const params = new URLSearchParams(formData);
    
    try {
        mostrarLoading();
        
        const response = await fetch(`/reportes/matriz/excel?${params}`, {
            method: 'GET',
            headers: {
...
async function generarExcelPedimentos() {
    ...
    formData.append('incluir_tiempos', document.getElementById('ped-incluir-tiempos').checked ? '1' : '0');
    formData.append('agrupar_cliente', document.getElementById('ped-agrupar-cliente').checked ? '1' : '0');
    ...
    try {
        ...
        const response = await fetch(`/reportes/pedimentos/excel?${params}`, {
            method: 'GET',
            headers: {
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
            }
...
function mostrarLoading() {
    document.getElementById('loading-reportes').classList.remove('hidden');
}

// Ocultar loading
function ocultarLoading() {
    document.getElementById('loading-reportes').classList.add('hidden');
}
