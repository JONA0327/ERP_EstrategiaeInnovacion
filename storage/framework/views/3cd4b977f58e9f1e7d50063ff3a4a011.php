                    ...
                    <?php if(isset($criterios) && $criterios->isNotEmpty()): ?>
                        <div class="space-y-10">
                            <?php $__currentLoopData = $criterios; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $criterio): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                ...
                                ?>
                                <div class="relative bg-slate-50 p-6 rounded-2xl border border-slate-100 hover:border-indigo-100 transition-colors" x-data="{ selected: <?php echo e($valPrevio ?? 'null'); ?> }">
                                    <div class="flex justify-between items-start mb-4">
                                        <div>
                                            <h4 class="text-lg font-bold text-slate-800"><?php echo e($criterio->criterio); ?></h4>
                                            <p class="text-sm text-slate-500 mt-1 max-w-3xl"><?php echo e($criterio->descripcion); ?></p>
                                        </div>
                                        <span class="text-xs font-bold bg-white text-slate-400 px-2 py-1 rounded border border-slate-200 shadow-sm">Peso: <?php echo e($criterio->peso); ?>%</span>
                                    </div>
                                        ...
                                        <?php $__currentLoopData = $opciones; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $op): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                            <label class="cursor-pointer group relative">
                                                ...
                                                <div class="h-full flex flex-col items-center justify-center p-3 rounded-xl border-2 border-white bg-white shadow-sm transition-all duration-200 peer-checked:border-indigo-500 peer-checked:bg-indigo-50 peer-checked:text-indigo-700 hover:shadow-md">
                                                    <span class="text-lg font-bold mb-1"><?php echo e($op['val']); ?></span>
                                                    <span class="text-[10px] uppercase font-bold tracking-wider opacity-70"><?php echo e($op['label']); ?></span>
                                                </div>
                                            </label>
                                    ...
                                    <div class="mt-4">
                                        <input type="text" name="observaciones[<?php echo e($criterio->id); ?>]" value="<?php echo e($obsPrevia); ?>" 
                                            class="w-full text-sm rounded-lg border-slate-200 focus:border-indigo-500 focus:ring-indigo-500 placeholder-slate-400" 
                                            placeholder="Observaciones específicas para este punto (opcional)..." <?php echo e($is_locked ? 'disabled' : ''); ?>>
                                    </div>
                                </div>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </div>
