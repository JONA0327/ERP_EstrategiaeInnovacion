...
<div class="min-h-screen bg-slate-50 pb-12">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <?php $__currentLoopData = ['success' => 'emerald', 'error' => 'red', 'info' => 'blue']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $color): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <?php if(session($key)): ?>
                <div class="mb-6 flex items-center p-4 bg-<?php echo e($color); ?>-50 border border-<?php echo e($color); ?>-200 rounded-2xl shadow-sm">
                    <div class="p-2 bg-<?php echo e($color); ?>-100 rounded-full text-<?php echo e($color); ?>-600 mr-3">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <?php if($key == 'success'): ?> <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                            <?php elseif($key == 'error'): ?> <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                            <?php else: ?> <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M12 18a9 9 0 110-18 9 9 0 010 18z"></path> <?php endif; ?>
                        </svg>
                    </div>
                    <p class="text-<?php echo e($color); ?>-800 font-medium"><?php echo e(session($key)); ?></p>
                </div>
            <?php endif; ?>
            ...
            <div class="bg-white rounded-[2rem] shadow-sm border border-amber-200 mb-10 overflow-hidden relative">
                ...
                <div class="px-8 py-6 bg-amber-50/50 border-b border-amber-100 flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-sm text-slate-500 mt-1"><?php echo e($pendingUsers->count()); ?> usuario(s) esperando aprobación de acceso.</p>
                    </div>
                </div>
                                ...
                                <tr class="hover:bg-amber-50/30 transition-colors">
                                    <td class="px-8 py-5">
                                        <div class="flex flex-col">
                                            <span class="font-bold text-slate-900"><?php echo e($user->name); ?></span>
                                            <span class="text-sm text-slate-500"><?php echo e($user->email); ?></span>
                                        </div>
                                    </td>
                                    <td class="px-8 py-5 text-sm text-slate-500">
                                        Solicitado <span class="font-bold"><?php echo e($user->created_at->diffForHumans()); ?></span>
                                    </td>
                                    <td class="px-8 py-5 text-right">
                                        <div class="flex justify-end gap-3">
                                            ...
                                            <form method="POST" action="<?php echo e(route('admin.users.reject', $user)); ?>" class="flex items-center gap-2">
                                                <?php echo csrf_field(); ?>
        ...
        <div class="bg-white rounded-[2rem] shadow-sm border border-slate-200 overflow-hidden">
            <div class="px-8 py-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                <div>
                    ...
                    <p class="text-sm text-slate-500"><?php echo e($approvedUsers->total()); ?> usuarios registrados.</p>
                </div>
            </div>
                                ...
                                <tr class="hover:bg-slate-50/80 transition-colors">
                                    <td class="px-8 py-4">
                                        <div class="flex items-center">
                                            ...
                                            <div>
                                                <div class="font-bold text-slate-900"><?php echo e($user->name); ?></div>
                                    ...
                                    <td class="px-8 py-4">
                                        <div class="flex flex-col">
                                            <span class="text-xs font-bold text-slate-700"><?php echo e($user->tickets()->count()); ?> tickets</span>
                                            <span class="text-[10px] text-slate-400">Reg: <?php echo e($user->created_at->format('d/m/Y')); ?></span>
                                        </div>
                                    </td>
                                    <td class="px-8 py-4 text-right">
                                        <div class="flex justify-end gap-2">
                                            <a href="<?php echo e(route('admin.users.edit', $user)); ?>" class="inline-flex items-center justify-center w-8 h-8 rounded-xl bg-white border border-slate-200 text-slate-400 hover:text-indigo-600 hover:border-indigo-200 hover:shadow-sm transition-all" title="Editar">
                                            ...
                                            <?php if($user->id !== auth()->id()): ?>
                                                <button class="inline-flex items-center justify-center w-8 h-8 rounded-xl bg-white border border-slate-200 text-slate-400 hover:text-red-600 hover:border-red-200 hover:shadow-sm transition-all"
                                                        ...
                                                        data-user-name="<?php echo e($user->name); ?>" title="Eliminar">
        ...
        <div class="grid md:grid-cols-2 gap-6 mt-8">
            <?php if($rejectedUsers->count() > 0): ?>
                <div class="bg-white rounded-[2rem] shadow-sm border border-slate-200 overflow-hidden">
                    ...
                    <div class="divide-y divide-slate-100">
                        <?php $__currentLoopData = $rejectedUsers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $user): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div class="px-6 py-4 flex justify-between items-center hover:bg-slate-50 transition-colors">
                                <div>
                                    <p class="text-sm font-bold text-slate-700"><?php echo e($user->name); ?></p>
                                    <p class="text-xs text-slate-400"><?php echo e($user->email); ?></p>
                                </div>
                                <form method="POST" action="<?php echo e(route('admin.users.rejections.destroy', $user)); ?>"
            ...
            <?php if($blockedEmails->count() > 0): ?>
                <div class="bg-white rounded-[2rem] shadow-sm border border-slate-200 overflow-hidden">
                    ...
                    <div class="divide-y divide-slate-100">
                        <?php $__currentLoopData = $blockedEmails; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $blocked): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div class="px-6 py-4 flex justify-between items-center hover:bg-slate-50 transition-colors">
                                <div>
                                    <p class="text-sm font-bold text-slate-700"><?php echo e($blocked->email); ?></p>
                                    <p class="text-xs text-slate-400"><?php echo e($blocked->reason ?? 'Sin motivo'); ?></p>
                                </div>
                                <form method="POST" action="<?php echo e(route('admin.blocked-emails.destroy', $blocked)); ?>">
                                    <?php echo csrf_field(); ?>
                                    <?php echo method_field('DELETE'); ?>
...
<?php echo $__env->make('layouts.master', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\trade\Desktop\Proyectos\ERP_EstrategiaeInnovacion\resources\views\Sistemas_IT/admin/users/index.blade.php ENDPATH**/ ?>