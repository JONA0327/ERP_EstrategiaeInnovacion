...
<?php $component->withAttributes([]); ?>
    ...
    <div class="relative py-8 min-h-screen z-10" 
         x-data="{ 
            ...
            currentActivity: null, currentActivityId: null, currentNotes: '', currentEvidenceUrl: '', currentStatus: '', historyLogs: [],
            ...
            openNotes(id, name, status, evidenceUrl) { 
                this.currentActivityId = id; 
                this.currentActivity = name; 
                this.currentStatus = status;
                this.currentEvidenceUrl = evidenceUrl;
                    ...
                    <form :action="'/activities/' + currentActivityId" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="bg-white px-6 pt-6 pb-4">
                            <div class="flex justify-between items-start mb-4">
                                <h3 class="text-xl font-bold text-slate-900">Editar Actividad y Evidencia</h3>
                            ...
                            <div class="mb-6">
                                ...
                                <select name="estatus" x-model="currentStatus" class="w-full text-sm border-slate-300 rounded-xl focus:ring-indigo-500 focus:border-indigo-500 shadow-sm cursor-pointer bg-white">
                                    ...
                                    <option value="Completado">🟢 Completado (Requiere Evidencia)</option>
                                </select>
                                <p class="text-[10px] text-red-400 mt-1 italic" x-show="currentStatus.includes('Completado') && !currentEvidenceUrl">
                                    * Recuerda subir un archivo antes de guardar como Completado.
                                </p>
                            </div>
                            ...
                            <div class="relative">
                                ...
                                <div class="absolute top-0 right-0 z-10">
                                    <button type="button" @click="appendSignature()" class="inline-flex items-center px-3 py-1 border border-indigo-200 text-[10px] font-bold rounded-full text-indigo-700 bg-indigo-50 hover:bg-indigo-100 focus:outline-none transition shadow-sm">
                                        ✍️ Firmar hoy
                                    </button>
                                </div>
                                <textarea id="big-note-area" name="comentarios" x-model="currentNotes" rows="6" class="shadow-inner focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-slate-300 rounded-xl font-mono bg-slate-50 p-4 leading-relaxed resize-none" placeholder="Escribe aquí los avances..."></textarea>
                            </div>
                        </div>

                        <div class="bg-slate-50 px-6 py-4 flex flex-row-reverse gap-3 border-t border-slate-100">
                            <button type="submit" class="w-full inline-flex justify-center rounded-xl border border-transparent shadow-md px-5 py-2.5 bg-indigo-600 text-base font-bold text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:ml-3 sm:w-auto sm:text-sm transition-transform active:scale-95">Guardar Todo</button>
