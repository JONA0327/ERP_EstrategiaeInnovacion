@php
    $user = Auth::user();
    $areaContext = request()->is('recursos-humanos*') ? 'RH' : (request()->is('logistica*') ? 'Logística' : 'ERP');
...
<nav x-data="{ open:false }" class="relative z-50 border-b border-slate-200 bg-white text-slate-700 shadow-md shadow-slate-200/70">
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 h-18 flex items-center justify-between">
        <a href="{{ $areaContext==='RH' ? route('recursos-humanos.index') : ($areaContext==='Logística' ? route('logistica.index') : route('welcome')) }}" class="flex items-center gap-3 py-3">
