<x-app-layout>
    {{-- FONDO ATMOSFÉRICO (Igual que en Welcome) --}}
                            ...
                            <tr class="hover:bg-indigo-50/40 transition-colors group {{ str_contains($act->estatus, 'Completado') ? 'bg-slate-50/80' : '' }}">
                                ...
                                <td class="px-3 py-3">
                                    <form action="{{ route('activities.update', $act->id) }}" method="POST">
                                        ...
                                        @endphp
                                        <select name="estatus" onchange="this.form.submit()" class="text-[10px] py-1 pl-2 pr-6 w-full rounded-md border {{ $statusStyle }} font-semibold shadow-sm focus:ring-2 focus:ring-indigo-500 cursor-pointer appearance-none">
                                            <option value="En blanco" {{ $act->estatus == 'En blanco' ? 'selected' : '' }}>⚪ Pendiente</option>
                                            <option value="En proceso" {{ $act->estatus == 'En proceso' ? 'selected' : '' }}>🔵 En proceso</option>
                                            <option value="Completado" {{ str_contains($act->estatus, 'Completado') ? 'selected' : '' }}>🟢 Listo</option>
                                            @if(in_array($act->estatus, ['Retardo', 'Completado con retardo']))
                                                <option value="{{ $act->estatus }}" selected disabled>
                                                    {{ $act->estatus == 'Retardo' ? '🔴 Retardo' : '🟠 Tardío' }}
                                                </option>
                                            @endif
                                        </select>
                                    </form>
