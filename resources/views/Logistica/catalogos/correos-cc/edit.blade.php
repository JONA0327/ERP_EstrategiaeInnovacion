    ...
    <div class="max-w-2xl mx-auto bg-white rounded-lg shadow-md">
        ...
        <div class="p-6 border-b border-gray-200">
            <div class="flex items-center">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">Editar Correo CC</h1>
                    <p class="text-gray-600">Modifica la configuración del correo: {{ $correoCC->nombre }}</p>
                </div>
            </div>
        </div>

        <!-- Formulario -->
        <form action="{{ route('logistica.correos-cc.update', $correoCC) }}" method="POST" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6">
                ...
                <input type="text" 
                       id="nombre" 
                       name="nombre" 
                       value="{{ old('nombre', $correoCC->nombre) }}"
            ...
            <div class="mb-6">
                ...
                <input type="email" 
                       id="email" 
                       name="email" 
                       value="{{ old('email', $correoCC->email) }}"
            ...
            <div class="mb-6">
                ...
                <select id="tipo" 
                    ...
                    <option value="administrador" {{ old('tipo', $correoCC->tipo) === 'administrador' ? 'selected' : '' }}>
                        Administrador
                    </option>
                    <option value="supervisor" {{ old('tipo', $correoCC->tipo) === 'supervisor' ? 'selected' : '' }}>
                        Supervisor
                    </option>
                    <option value="notificacion" {{ old('tipo', $correoCC->tipo) === 'notificacion' ? 'selected' : '' }}>
                        Notificación
                    </option>
                </select>
                @error('tipo')
            ...
            <div class="mb-6">
                ...
                <textarea id="descripcion" 
                          ...
                          placeholder="Descripción opcional del rol o propósito de este correo...">{{ old('descripcion', $correoCC->descripcion) }}</textarea>
                @error('descripcion')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-6">
                <div class="flex items-center">
                    <input type="checkbox" 
                           ...
                           name="activo" 
                           value="1"
                           {{ old('activo', $correoCC->activo) ? 'checked' : '' }}
                ...
                <div class="flex">
                    ...
                    <div class="ml-3">
                        <p class="text-sm text-blue-700">
                            <strong>Creado:</strong> {{ $correoCC->created_at->format('d/m/Y H:i') }}
                            @if($correoCC->updated_at && $correoCC->updated_at != $correoCC->created_at)
                                <br><strong>Última modificación:</strong> {{ $correoCC->updated_at->format('d/m/Y H:i') }}
                            @endif
                        </p>
                    </div>
                </div>
            ...
            <div class="flex justify-end space-x-4 pt-6 border-t border-gray-200">
                ...
                <button type="submit" 
                    ...
                    <i class="fas fa-save mr-2"></i>Actualizar Correo CC
                </button>
            </div>
        </form>
    </div>
